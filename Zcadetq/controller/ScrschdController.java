...
public class ScrschdController extends DefaultBlockController {
	...
	//action methods generated from triggers
		...
		/* Original PL/SQL code code for TRIGGER SCRSCHD.PRE-QUERY
		    :SCHD_CTRL.SCRSCHD_TERM_CODE_FROM :=
...
--
   DECLARE
      CURSOR PTI_CURSOR IS
         ...
         FROM   SCRSCHD
         WHERE  SCRSCHD_SUBJ_CODE = :KEY_BLOCK.SCACRSE_SUBJ_CODE
           AND  SCRSCHD_CRSE_NUMB = :KEY_BLOCK.SCACRSE_CRSE_NUMB
           AND  SCRSCHD_EFF_TERM > :KEY_BLOCK.STVTERM_CODE ;
   BEGIN
      OPEN PTI_CURSOR ;
      FETCH PTI_CURSOR INTO :SCHD_CTRL.SCRSCHD_TERM_CODE_TO ;
   END ;
--
   IF :SCHD_CTRL.SCRSCHD_TERM_CODE_TO IN (
      :SCHD_CTRL.SCRSCHD_TERM_CODE_FROM) OR
      :SCHD_CTRL.SCRSCHD_TERM_CODE_TO IS NULL  OR (
      :SCHD_CTRL.SCRSCHD_TERM_CODE_TO IS NULL AND
      :SCHD_CTRL.SCRSCHD_TERM_CODE_FROM IS NULL ) THEN
      GOTO GET_SCBCRKY ;
   ELSE
      RETURN ;
   END IF ;
--
   ...
   IF SELECT_SCBCRKY_1 (:KEY_BLOCK.SCACRSE_SUBJ_CODE,
      ...
      :SCHD_CTRL.SCRSCHD_TERM_CODE_TO) THEN
      GOTO PTI_STEP_001;
   END IF ;
--
	 << PTI_STEP_001 >>
   IF :SCHD_CTRL.SCRSCHD_TERM_CODE_TO IS NULL  THEN
      :SCHD_CTRL.SCRSCHD_TERM_CODE_TO := :TERM_CODE_END ;
   END IF ;
		*/
		/*
		 *<p>
		...
		*/
		...
		public void scrschd_BeforeQuery(QueryEvent args)
		{
				...
				Ref<NString> termCode = new Ref<NString>(getFormModel().getSchdCtrl().getScrschdTermCodeTo());
				...
				getFormModel().getSchdCtrl().setScrschdTermCodeFrom(scbcrseElement.getScbcrseEffTerm());
				{
					String sqlptiCursor = "SELECT MIN(SCRSCHD_EFF_TERM) " +
	" FROM SCRSCHD " +
	" WHERE SCRSCHD_SUBJ_CODE = :KEY_BLOCK_SCACRSE_SUBJ_CODE AND SCRSCHD_CRSE_NUMB = :KEY_BLOCK_SCACRSE_CRSE_NUMB AND SCRSCHD_EFF_TERM > :KEY_BLOCK_STVTERM_CODE ";
					...
					try {
						...
						if ( ptiCursorResults != null ) {
							getFormModel().getSchdCtrl().setScrschdTermCodeTo(ptiCursorResults.getStr(0));
						}
					}
					finally{
						ptiCursor.close();
				...
				// 
				if ( !(in(getFormModel().getSchdCtrl().getScrschdTermCodeTo(), getFormModel().getSchdCtrl().getScrschdTermCodeFrom()).getValue() || getFormModel().getSchdCtrl().getScrschdTermCodeTo().isNull() || (getFormModel().getSchdCtrl().getScrschdTermCodeTo().isNull() && getFormModel().getSchdCtrl().getScrschdTermCodeFrom().isNull())) )
					return;
				...
				{
					getFormModel().getSchdCtrl().setScrschdTermCodeTo(termCode.val);
				}
					getFormModel().getSchdCtrl().setScrschdTermCodeTo(termCode.val);
				if ( getFormModel().getSchdCtrl().getScrschdTermCodeTo().isNull() )
				{
					getFormModel().getSchdCtrl().setScrschdTermCodeTo(getFormModel().getFormHeader().getTermCodeEnd());
				}
			}
      ...
      :SCRSCHD.SCRSCHD_EFF_TERM ;
		...
		*/
		...
		public void scrschd_AfterQuery(RowAdapterEvent args)
		{
				...
				getFormModel().getSchdCtrl().setScrschdTermCodeFrom(scrschdElement.getScrschdEffTerm());
			}
...
--
   DECLARE
      CURSOR PTI_CURSOR IS
         SELECT STVSCHD_DESC
         FROM   STVSCHD
         WHERE  STVSCHD_CODE = :SCRSCHD.SCRSCHD_SCHD_CODE ;
   BEGIN
      OPEN PTI_CURSOR ;
      FETCH PTI_CURSOR INTO :SCRSCHD.STVSCHD_DESC ;
   END ;
--
   :GLOBAL.QUERY_MODE := '0' ;
EXCEPTION
...
END ;
	...
	" FROM STVSCHD " +
	" WHERE STVSCHD_CODE = :SCRSCHD_SCRSCHD_SCHD_CODE ";
						DataCursor ptiCursor = new DataCursor(sqlptiCursor);
						try {
							//Setting query parameters
							ptiCursor.addParameter("SCRSCHD_SCRSCHD_SCHD_CODE", scrschdElement.getScrschdSchdCode());
							ptiCursor.open();
							ResultSet ptiCursorResults = ptiCursor.fetchInto();
							if ( ptiCursorResults != null ) {
								scrschdElement.setStvschdDesc(ptiCursorResults.getStr(0));
							}
						}
						finally{
							ptiCursor.close();
		 ...
		 *<b>Migration Comments</b>
		 * Generated from trigger:
		 * STVSCHD_DESC.WHEN-NEW-ITEM-INSTANCE
		 *
		 *
		 *</p>
		*/

		@ActionTrigger(action="WHEN-NEW-ITEM-INSTANCE", item="STVSCHD_DESC", function=KeyFunction.ITEM_CHANGE)
		public void stvschdDesc_itemChange()
		{
